...
#taxData = taxData |> 
  #filter(`SITUS STATE` %in% statenewabb)
TaxData=TaxData|>select(`YEAR BUILT`,`ACRES`,`LAND SQUARE FOOTAGE`,`UNIVERSAL BUILDING SQUARE FEET`,`UNIVERSAL BUILDING SQUARE FEET SOURCE INDICATOR CODE`,`BUILDING SQUARE FEET`,`BEDROOMS - ALL BUILDINGS`,`TOTAL BATHROOMS - ALL BUILDINGS`,`NUMBER OF BATHROOMS`,`FULL BATHS - ALL BUILDINGS`,`HALF BATHS - ALL BUILDINGS`,`TAX AMOUNT`,`TAX YEAR`,`ROOF TYPE CODE`,`FIPS CODE`,`CLIP`,`SITUS STATE`,`SALE DATE`,`SITUS ZIP CODE`,`SITUS CITY`)
fullData=fullData |>select(`FIPS CODE`,`CLIP`,`DEED SITUS STATE - STATIC`,`DEED SITUS COUNTY - STATIC`,`SALE DERIVED DATE`,`PRIMARY CATEGORY CODE`,`SALE AMOUNT`,`DEED SITUS ZIP CODE - STATIC`)
...
TaxDeedsData=TaxDeedsData|>mutate("YEAR"=substr(`SALE DERIVED DATE`,1,4)) |>arrange(desc(`YEAR`))|>collect()
TaxDeedsData=TaxDeedsData|> distinct(`CLIP`,.keep_all = TRUE)
...
TaxDeedsData=TaxDeedsData|>filter(!is.na(`ACRES`)) |> filter(!is.na(`NUMBER OF BATHROOMS`)) |> filter(!is.na(`UNIVERSAL BUILDING SQUARE FEET`)) |> filter(!is.na(`BEDROOMS - ALL BUILDINGS`)) |>filter(!is.na(`YEAR BUILT`))|>filter(!is.na(`YEAR`))
TaxDeedsData = TaxDeedsData |> mutate(`ACRES` = as.numeric(`ACRES`)) |>
  mutate(`NUMBER OF BATHROOMS` = as.numeric(`NUMBER OF BATHROOMS`)) |> mutate(`UNIVERSAL BUILDING SQUARE FEET`=as.numeric(`UNIVERSAL BUILDING SQUARE FEET`))|> mutate(`BEDROOMS - ALL BUILDINGS`=as.numeric(`BEDROOMS - ALL BUILDINGS`)) |>mutate(`YEAR BUILT`=as.numeric(`YEAR BUILT`)) |>mutate("YEAR"=as.numeric(`SALE DATE`)) 
...
print(TrainData)
print(TrialData)
print(SecondTrialData)

#LinearModel=lm(`SALE AMOUNT`~`ACRES`+`NUMBER OF BATHROOMS`+`UNIVERSAL BUILDING SQUARE FEET`+`BEDROOMS - ALL BUILDINGS`+`YEAR BUILT`+`YEAR`,data=TrainData)

#print(LinearModel)
#190000.00
RandomHouse=c(TrialData[1,c(2,9,4,7,1,27)])
#print(RandomHouse)
#print(predict(LinearModel,newdata=RandomHouse))
