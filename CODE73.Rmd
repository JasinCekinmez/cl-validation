...
fullData <- fullData %>%
  ...
  select(`FIPS CODE`, `CLIP`, `DEED SITUS STATE - STATIC`, `DEED SITUS COUNTY - STATIC`, `SALE DERIVED DATE`, `PRIMARY CATEGORY CODE`, `SALE AMOUNT`, `DEED SITUS ZIP CODE - STATIC`, `DEED CATEGORY TYPE CODE`)

TaxData <- TaxData %>%
  filter(`FIPS CODE` == FIPS) %>%
...
TaxDeedsData <- left_join(TaxData, fullData, by = c("CLIP", "FIPS CODE")) %>%
  ...
  mutate(`ACRES` = as.numeric(`ACRES`),
         `NUMBER OF BATHROOMS` = as.numeric(`NUMBER OF BATHROOMS`),
         `UNIVERSAL BUILDING SQUARE FEET` = as.numeric(`UNIVERSAL BUILDING SQUARE FEET`),
  ...
  filter(!is.na(`NUMBER OF BATHROOMS`),
...
LinearModel <- lm(`SALE AMOUNT` ~ `NUMBER OF BATHROOMS` + `BEDROOMS - ALL BUILDINGS` + `YEAR BUILT` + `YEAR` + I((`YEAR`)^2) + `LIVING SQUARE FEET - ALL BUILDINGS` + `FEDERAL TRACT` + `PROPERTY INDICATOR CODE`, data = TrainData)
...
params =tibble(FIPS="39035")
slurm_apply(f=Slurmmaker,params=params,jobname = "CUYAHOGA",nodes = 19,
              cpus_per_node = 1,
...
fullData = fullData |> filter(`DEED SITUS STATE - STATIC`=="WY")  |>filter(`DEED SITUS COUNTY - STATIC`=="SWEETWATER")
...
fullData = fullData |> filter(`DEED SITUS STATE - STATIC`==STATE) 
...
ggsave("~/cl-validation/Output/UTAHNOOUTLIERS.jpg",plot = last_plot())
# Disconnect the database
dbDisconnect(con)
}

params=tibble(STATE="UT")
slurm_apply(f=Slurmmake,params=params,jobname = "UT",nodes = 19,
              cpus_per_node = 1,
              slurm_options = list(time = '1:05:00',
                                   'mem-per-cpu' = '99G',
                                   'mail-type' = list('begin', 'end', 'fail'),
...
SanData=TaxData|> mutate("Tract"=substr(`CENSUS ID`,1,6),.before = 1) |>select(`Tract`)|>collect()
##|>group_by(`Tract`)|>summarize("Amount"=n())
#FullData=FullData |> filter(`APN (PARCEL NUMBER UNFORMATTED)`=="33791932800200"&`DEED SITUS CITY - STATIC`=="CASPER")
#TaxData=TaxData|>filter(`SITUS STATE`=="CA") |> filter(`SITUS CITY`=="LEMON GROVE") |> summarise(n=n())
#print(TaxData)
#ggplot(SanData,aes(y=`Tract`))+geom_bar()
#TaxData=TaxData|>group_by(`COUNTY USE DESCRIPTION`)|>count() |> filter(n>10000)
TaxData=TaxData|> mutate("FEDERAL TRACT"=paste0(`FIPS CODE`,substr(`CENSUS ID`,1,6)),.before = 1)
KernData=TaxData |> filter(`FEDERAL TRACT`=="06029002100")
print(KernData)
#print(TaxData)
#print(SanteeData)
#print(ElCajonData)
