...
#taxData=taxData|>select(`YEAR BUILT`,`ACRES`,`LAND SQUARE FOOTAGE`,`UNIVERSAL BUILDING SQUARE FEET`,`UNIVERSAL BUILDING SQUARE FEET SOURCE INDICATOR CODE`,`BUILDING SQUARE FEET`,`BEDROOMS - ALL BUILDINGS`,`TOTAL BATHROOMS - ALL BUILDINGS`,`NUMBER OF BATHROOMS`,`FULL BATHS - ALL BUILDINGS`,`HALF BATHS - ALL BUILDINGS`,`FIPS CODE`,`CLIP`,`APN (PARCEL NUMBER UNFORMATTED)`,`ORIGINAL APN`,`APN SEQUENCE NUMBER`,`SITUS STATE`)
taxData = taxData |> 
...
TaxDeedsData=TaxDeedsData|>distinct(`CLIP`,.keep_all = TRUE)
...
##This is what my code looked to generate the csv files 
#tibbleCreator("UNIVERSAL BUILDING SQUARE FEET","DEED SITUS COUNTY - STATIC",FALSE,TRUE,TRUE)
#tibbleCreator("UNIVERSAL BUILDING SQUARE FEET","DEED SITUS STATE - STATIC",FALSE,TRUE,TRUE)

#tibbleCreator("FULL BATHS - ALL BUILDINGS","DEED SITUS COUNTY - STATIC",FALSE,TRUE,TRUE)
#tibbleCreator("FULL BATHS - ALL BUILDINGS","DEED SITUS STATE - STATIC",FALSE,TRUE,TRUE)
#tibbleCreator("HALF BATHS - ALL BUILDINGS","DEED SITUS COUNTY - STATIC",FALSE,TRUE,TRUE)
#tibbleCreator("HALF BATHS - ALL BUILDINGS","DEED SITUS STATE - STATIC",FALSE,TRUE,TRUE)
...
TaxData=TaxData|> 
 filter( `SITUS STATE`%in% statenewabb) #|> mutate("ZIPCODE"=substr(`SITUS ZIP CODE`,1,5))|>filter(`ZIPCODE`=="92071") |>collect()
TaxData=TaxData|> filter(`SITUS STATE`=="CA")|> filter(`SITUS CITY`=="SAN DIEGO") |>collect()
print(TaxData)
...
#Takes the database connection and turns it into a tibble and filters out non states and DC and ones that do not exist
#fullData = fullData |> 
 #   filter(`DEED SITUS STATE - STATIC` %in% statenewabb) #|> filter(`DEED SITUS CITY - STATIC`=="SANTEE")|> filter(`DEED SITUS STATE - STATIC`=="CA")

#taxData = taxData |> 
  #filter(`SITUS STATE` %in% statenewabb)
TaxData=TaxData|>select(`YEAR BUILT`,`ACRES`,`LAND SQUARE FOOTAGE`,`UNIVERSAL BUILDING SQUARE FEET`,`UNIVERSAL BUILDING SQUARE FEET SOURCE INDICATOR CODE`,`BUILDING SQUARE FEET`,`BEDROOMS - ALL BUILDINGS`,`TOTAL BATHROOMS - ALL BUILDINGS`,`NUMBER OF BATHROOMS`,`FULL BATHS - ALL BUILDINGS`,`HALF BATHS - ALL BUILDINGS`,`TAX AMOUNT`,`TAX YEAR`,`ROOF TYPE CODE`,`FIPS CODE`,`CLIP`,`SITUS STATE`,`SALE DATE`,`SITUS ZIP CODE`,`SITUS CITY`,`LIVING SQUARE FEET - ALL BUILDINGS`,`COUNTY USE DESCRIPTION`,`CENSUS ID`,`SITUS STREET NAME`,`SITUS HOUSE NUMBER`,`APN (PARCEL NUMBER UNFORMATTED)`)
fullData=fullData|> filter(`DEED CATEGORY TYPE CODE`!="U")
fullData=fullData |>select(`FIPS CODE`,`CLIP`,`DEED SITUS STATE - STATIC`,`DEED SITUS COUNTY - STATIC`,`SALE DERIVED DATE`,`PRIMARY CATEGORY CODE`,`SALE AMOUNT`,`DEED SITUS ZIP CODE - STATIC`,`DEED CATEGORY TYPE CODE`)
TaxData=TaxData |>filter(`SITUS STATE`=="CA") |> filter(`SITUS CITY`=="SANTEE")
TaxDeedsData = left_join(TaxData, fullData, by = c("CLIP", "FIPS CODE")) 

#TaxDeedsData=TaxDeedsData|>distinct(`CLIP`,.keep_all = TRUE) |>mutate("YEAR"=substr(`SALE DERIVED DATE`,1,4)) #|>
  #collect() #|>sample_frac(0.6)
TaxDeedsData=TaxDeedsData|>mutate("YEAR"=substr(`SALE DERIVED DATE`,1,4))|> mutate("CENSUS TRACT"=substr(`CENSUS ID`,1,6))
TaxDeedsData = TaxDeedsData |> mutate(`ACRES` = as.numeric(`ACRES`)) |>
  mutate(`NUMBER OF BATHROOMS` = as.numeric(`NUMBER OF BATHROOMS`)) |> mutate(`UNIVERSAL BUILDING SQUARE FEET`=as.numeric(`UNIVERSAL BUILDING SQUARE FEET`))|> mutate(`BEDROOMS - ALL BUILDINGS`=as.numeric(`BEDROOMS - ALL BUILDINGS`)) |>mutate(`YEAR BUILT`=as.numeric(`YEAR BUILT`)) |>mutate(`YEAR`=as.numeric(`YEAR`)) |>mutate(`SALE AMOUNT`=as.numeric(`SALE AMOUNT`)) |>mutate(`LIVING SQUARE FEET - ALL BUILDINGS`=as.numeric(`LIVING SQUARE FEET - ALL BUILDINGS`)) 
TaxDeedsData=TaxDeedsData |> filter(!is.na(`NUMBER OF BATHROOMS`)) |> filter(!is.na(`UNIVERSAL BUILDING SQUARE FEET`)) |> filter(!is.na(`BEDROOMS - ALL BUILDINGS`)) |>filter(!is.na(`YEAR BUILT`))|>filter(!is.na(`YEAR`)) |>filter(!is.na(`SALE AMOUNT`)) |>filter(!is.na(`LIVING SQUARE FEET - ALL BUILDINGS`))|>filter(!is.na(`ROOF TYPE CODE`))|>filter(!is.na(`COUNTY USE DESCRIPTION`)) |>filter(!is.na(`UNIVERSAL BUILDING SQUARE FEET SOURCE INDICATOR CODE`)) |>filter(!is.na(`CENSUS TRACT`))
TaxDeedsData=TaxDeedsData|>arrange(desc(`CLIP`),desc(`SALE DERIVED DATE`)) |>collect()


TaxDeedsData=TaxDeedsData|> distinct(`CLIP`,.keep_all = TRUE)

#=TaxDeedsData = TaxDeedsData |> mutate(`ACRES` = as.numeric(ACRES)) |>
#  mutate(`NUMBER OF BATHROOMS` = as.numeric(`NUMBER OF BATHROOMS`)) |> mutate(`UNIVERSAL BUILDING SQUARE FEET`=as.numeric(`UNIVERSAL BUILDING SQUARE FEET`))|> mutate(`BEDROOMS - ALL BUILDINGS`=as.numeric(`BEDROOMS - ALL BUILDINGS`)) |>mutate(`YEAR BUILT`=as.numeric(`YEAR BUILT`)) |>mutate("YEAR"=as.numeric(`SALE DATE`)) 
...
#`UNIVERSAL BUILDING SQUARE FEET SOURCE INDICATOR CODE` normally would effect but here they are L's

LinearModel=lm(`SALE AMOUNT`~`NUMBER OF BATHROOMS`+`BEDROOMS - ALL BUILDINGS`+`YEAR BUILT`+`YEAR`+I((`YEAR`)^2)+`LIVING SQUARE FEET - ALL BUILDINGS`+`COUNTY USE DESCRIPTION`+`CENSUS TRACT`,data=TrainData)
#LinearMode=lm(`SALE AMOUNT`~`LIVING SQUARE FEET - ALL BUILDINGS`,data=TrainData)
#print(LinearModel)

#RandomHouse=c(TrialData[c(nrow(TrialData)),c(1-35)])
#print(RandomHouse)
...
#plot(`SALE AMOUNT`~`LIVING SQUARE FEET - ALL BUILDINGS`,data=TrainData) 
#abline(LinearMode, col = "blue") 

#ggplot(TaxData,aes(x=log(as.numeric(`MARKET TOTAL VALUE`))))+geom_histogram() + labs(title = "Home Prices in Las Vegas,NV",x="Market Total Value")

#ggplot(TaxData,aes(x=log(as.numeric(`ASSESSED TOTAL VALUE`))))+geom_histogram() + labs(title = "Home Prices in Las Vegas,NV",x="Assessed Total Value")
dbDisconnect(con)
````
````{R}
...
TaxData=tbl(con,"tax")
FullData=tbl(con,"deeds")
#SanteeData=TaxData|>filter(`SITUS STATE`=="CA")|>filter(`SITUS CITY`=="SANTEE") 
#ElCajonData=TaxData|>filter(`SITUS STATE`=="CA")|>filter(`SITUS CITY`=="EL CAJON") |> mutate("Tract"=substr(`CENSUS ID`,1,),.before = 1)
FullData=FullData |> filter(`APN (PARCEL NUMBER UNFORMATTED)`=="33791932800200"&`DEED SITUS CITY - STATIC`=="CASPER")
...
ggsave(filename="~/cl-validation/Output/LollipopTaxTrial.jpg",plot=last_plot())
dbDisconnect(con)
}
...
if(graph){
 ggplot(LollipopData) + 
  ...
  ggsave("~/cl-validation/Output/NewLollipop.pdf",last_plot(),dpi = 300,width = 6,height=4.5)
...
ArmsLengthFull=LollipopCreator("PRIMARY CATEGORY CODE",45,51,"","A","",TRUE,TRUE,FALSE)
...
scale_fill_distiller(palette = "RdYlGn", direction = 1) +
  ...
  theme_void()
ggsave("~/cl-validation/Output/norm.pdf",last_plot(),dpi = 300, width = 6 ,height =3)
...
scale_fill_distiller(palette = "RdYlGn", direction = 1) +
  ...
  theme_void()
ggsave("~/cl-validation/Output/Book.pdf",last_plot(),dpi = 300, width = 6 ,height = 3)
````
...
fullData=tbl(con,"deeds")
taxData=tbl(con,"tax")
#confull= dbConnect(duckdb(),"/projects/SHARKEY/corelogic_pu/CorelogicDeeds/2023Update/CopyOfcl.db")
#fData=tbl(confull,"deeds")
#tData=tbl(confull,"tax")

taxData=taxData|>select(`ACRES`,`FIPS CODE`,`CLIP`,`SITUS STATE`,`SALE DATE`)
fullData=fullData |>select(`FIPS CODE`,`CLIP`,`DEED SITUS STATE - STATIC`,`DEED SITUS COUNTY - STATIC`,`SALE DERIVED DATE`)
#print(fullData)
#ALData=taxData|>filter(`SITUS STATE`=="AL")
#print(ALData)

# Perform the join on in-memory data
TaxDeedsData = left_join(taxData, fullData, by = c("CLIP", "FIPS CODE")) |> mutate(Year=substr(`SALE DATE`,1,4),.after=`SALE DATE` ) 
TaxDeedsData=TaxDeedsData|>distinct(`CLIP`,.keep_all = TRUE)
#print(TaxDeedsData)
#TaxDeedsData= TaxDeedsData|>arrange(desc(`year`))
#print(TaxDeedsData)

#TaxDeedsData = left_join(fData, tData, by = c("CLIP", "FIPS CODE")) 
#print(TaxDeedsData)
