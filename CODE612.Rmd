...
#QuartileData=fullData |> group_by(`DEED SITUS STATE - STATIC`) |> filter(`DEED SITUS STATE - STATIC`!="NA")|> select(`SALE DERIVED DATE`) |> mutate("SALE DERIVED YEAR"=substr(`SALE DERIVED DATE`,1,4)) |> select(-`SALE DERIVED DATE`) |> arrange(desc(`SALE DERIVED YEAR`),desc(`DEED SITUS STATE - STATIC`))
...
#print(QuantileStateData)

QuantileCountyData= fullData |> filter(`DEED SITUS STATE - STATIC`!="NA",`DEED SITUS COUNTY - STATIC`!="NA") |> select(`SALE DERIVED DATE`,`DEED SITUS COUNTY - STATIC` ,`DEED SITUS STATE - STATIC`) |> mutate("SALE DERIVED YEAR"=substr(`SALE DERIVED DATE`,1,4)) |> group_by(`DEED SITUS COUNTY - STATIC`,`DEED SITUS STATE - STATIC`) |> summarize(quantile2.5Year = quantile(as.numeric(`SALE DERIVED YEAR`), na.rm = T, probs = c(.025)))
print(QuantileCountyData)
...
#med=median(pull(QuartileData,`SALE DERIVED YEAR`),na.rm = TRUE)
#print(med)
