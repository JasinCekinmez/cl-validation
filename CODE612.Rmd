...
MissingColumnValue=fullData |> filter(column!=type) |> group_by(`DEED SITUS STATE - STATIC`) |> summarise(n=n()) |> arrange(desc(`DEED SITUS STATE - STATIC`)) |> pull(n)
...
MissingColumn=fullData |> filter(column!=type) 
PresentColumn=fullData |> filter(column==type)
MissingColumnValue=fullData |> filter(column!=type) |> group_by(`DEED SITUS STATE - STATIC`,`FIPS CODE`)  |> summarise(n=n(), .groups = 'drop') |> arrange(desc(`DEED SITUS STATE - STATIC`),desc(`FIPS CODE`)) |> pull(n)
PresentColumnValue=fullData |> filter(column==type) |> group_by(`DEED SITUS STATE - STATIC`,`FIPS CODE`)  |> summarise(n=n(), .groups = 'drop') |>  arrange(desc(`DEED SITUS STATE - STATIC`),desc(`FIPS CODE`))|> pull(n)    
...
ValuePresentNonColumn= MissingColumn |> filter(!is.na(.data[[ColumnNameList[i]]])) |> group_by(`DEED SITUS STATE - STATIC`)  |> summarise(n=n())  |>
  collect() |> arrange(desc(`DEED SITUS STATE - STATIC`)) |>
  mutate("Not A" = as.numeric(n) / as.numeric(MissingColumnValue[1:length(n)])) |>mutate("Column"=ColumnNameList[i])


ValuePresentColumn=PresentColumn |> filter(!is.na(.data[[ColumnNameList[i]]])) |> group_by(`DEED SITUS STATE - STATIC`) |> summarise(n=n()) |> collect()|> arrange(desc(`DEED SITUS STATE - STATIC`)) |>
  mutate("A" = as.numeric(n) / as.numeric(PresentColumnValue[1:length(n)])) |>mutate("Column"=ColumnNameList[i])

Difference=left_join(ValuePresentNonColumn,ValuePresentColumn,by=c("DEED SITUS STATE - STATIC","Column"))|>mutate(difference=`A`-`Not A`) |> mutate(Difference=case_when(
   difference>0 ~ 1,
   difference<=0 ~ 0
 )) |>select(-c(`n.x`,`Not A`,`n.y`,`A`))
if(i==1){
Binder=Difference
allData=Difference
}
if(i!=1){
allData=bind_rows(Binder,Difference) 
Binder=allData
}
}
else if(!state &!graph){
ValuePresentNonColumn= MissingColumn |> filter(!is.na(.data[[ColumnNameList[i]]])) |> group_by(`DEED SITUS STATE - STATIC`,`FIPS CODE`)  |> summarise(n=n(), .groups = 'drop') |>
  collect() |> arrange(desc(`DEED SITUS STATE - STATIC`),desc(`FIPS CODE`)) |>
  mutate("Not A" = as.numeric(n) / as.numeric(MissingColumnValue[1:length(n)])) |>mutate("Column"=ColumnNameList[i])


ValuePresentColumn=PresentColumn |> filter(!is.na(.data[[ColumnNameList[i]]])) |> group_by(`DEED SITUS STATE - STATIC`,`FIPS CODE`) |> summarise(n=n(), .groups = 'drop') |> collect()|> arrange(desc(`DEED SITUS STATE - STATIC`),desc(`FIPS CODE`)) |>
  mutate("A" = as.numeric(n) / as.numeric(PresentColumnValue[1:length(n)])) |>mutate("Column"=ColumnNameList[i])

Difference=left_join(ValuePresentNonColumn,ValuePresentColumn,by=c("DEED SITUS STATE - STATIC","Column","FIPS CODE"))|>mutate(difference=`A`-`Not A`) |> mutate(Difference=case_when(
   difference>0 ~ 1,
   difference<=0 ~ 0
 )) |>select(-c(`n.x`,`Not A`,`n.y`,`A`))
if(i==1){
...
ArmsLengthFullFips=LollipopCreator("PRIMARY CATEGORY CODE",47,49,"","A","",FALSE,FALSE,FALSE)
print(ArmsLengthFullFips)
write.csv(ArmsLengthFullFips,"~/cl-validation/Output/ArmsLengthFullFips.csv")
#disconnects in order to avoid garbage collecting 
dbDisconnect(con)
````
