...
StateYearData=fullData |> filter(`DEED SITUS STATE - STATIC` != "NA" ) |> group_by(substr(`SALE DERIVED DATE`,1,4),`DEED SITUS STATE - STATIC`) |> summarize(n=n())
#print(StateYearData)

CountyYearData=fullData |> filter(`DEED SITUS STATE - STATIC` != "NA" ) |> group_by(substr(`SALE DERIVED DATE`,1,4),`DEED SITUS COUNTY - STATIC`,`DEED SITUS STATE - STATIC`) |> summarize(n=n())
...
data = (fullData |> group_by(region) |>filter(column !="NA") |> summarize(!!colu := n()))
...
data= data |> rename("Total Deeds" = `n`)
#changes column names 


data= data |> mutate("% missing"= round((100* (1-(!!sym(colu))/(`Total Deeds`))), digits=2)) 
...
data=fullData |> filter(column!="NA") |> filter(`DEED SITUS STATE - STATIC` != "NA") |> group_by(substr(`SALE DERIVED DATE`, 1, 4),`DEED SITUS STATE - STATIC`) |> summarize(!!colu := n())
region=toString(region)

data  = left_join(data, StateYearData, by=c("substr(`SALE DERIVED DATE`, 1, 4)","DEED SITUS STATE - STATIC"))
data= data |> rename("Total Deeds" = `n`)
data= data |> rename(`SALE DERIVED YEAR` = `substr(\`SALE DERIVED DATE\`, 1, 4)`)

data= data |> mutate("% missing"= round((100* (1-(!!sym(colu))/(`Total Deeds`))), digits=2)) 
#Now gives the missingness 
data=data |>  arrange(desc(`DEED SITUS STATE - STATIC`),desc(`SALE DERIVED YEAR`))

#CREATES CSV File
column=toString(column)
csvtitle=paste0(column,region,"YEAR.csv")
write.csv(data,csvtitle)
...
data = fullData |>filter(column != "NA") |> filter(`DEED SITUS STATE - STATIC` != "NA" ) |> group_by(region,`DEED SITUS STATE - STATIC`)  |> summarize(!!colu := n()) 
...
data= data |> mutate("% missing"= round((100* (1-(!!sym(colu))/(`Total Deeds`))), digits=2)) 
#Now gives the missingness 
data=data |>  arrange(desc(`DEED SITUS STATE - STATIC`),desc(`DEED SITUS COUNTY - STATIC`))
...
data=fullData |> filter(column!="NA") |> filter(`DEED SITUS STATE - STATIC` != "NA") |> group_by(substr(`SALE DERIVED DATE`, 1, 4),`DEED SITUS COUNTY - STATIC`,`DEED SITUS STATE - STATIC`) |> summarize(!!colu := n())
region=toString(region) 
...
data= data |> mutate("% missing"= round((100* (1-(!!sym(colu))/(`Total Deeds`))), digits=2)) 
#Now gives the missingness 
#data=data |>  arrange(desc(`SALE DERIVED YEAR`),desc(`DEED SITUS COUNTY - STATIC`) ,desc(`DEED SITUS STATE - STATIC`))
data=data |>  arrange(desc(`DEED SITUS STATE - STATIC`),desc(`DEED SITUS COUNTY - STATIC`) ,desc(`SALE DERIVED YEAR`))
